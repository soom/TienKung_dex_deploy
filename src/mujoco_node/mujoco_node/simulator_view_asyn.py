...
def quaternion_to_euler_scipy(w, x, y, z, degrees=False):
    import math
    if math.sqrt(w*w + x*x + y*y + z*z) < 1e-6:
        return 0.0, 0.0, 0.0
...
class RobotSimulator:
    ...
    def __init__(self, model_path, node: Node | None = None, robot_config='full'):
        # 加载模型
        ...
        # 启动可视化器
        self.paused = False
        # 禁用 viewer 的 GLFW 键盘回调（UP/DOWN/ESC 等），遥控器接管所有运行控制
        self.viewer = mujoco.viewer.launch_passive(self.model, self.data, show_left_ui=False, show_right_ui=False)
        self.viewer.cam.lookat = self.data.xpos[self.model.body("pelvis").id]
        ...
        if self.node:
            ...
            self.imu_status_pub = self.node.create_publisher(
                Imu, 'imu/status', 10)
            # 遥控器订阅：响应 rl_control 的 C/D/A 状态控制
            self.node.create_subscription(Joy, '/sbus_data', self._joy_callback, 10)
            # 弹性带远程控制（来自 sim_joy GUI）
            self.node.create_subscription(String, '/elastic_band_cmd', self._elastic_cmd_callback, 10)
            # 弹性带状态发布（供 sim_joy GUI 显示）
            self._elastic_status_pub = self.node.create_publisher(String, '/elastic_band_status', 10)
            self.node.create_timer(0.1, self._publish_elastic_status)

        # 仿真生命周期标志（受 /sbus_data 控制）
        self.simulation_started = False
        self.trigger_reset = False
        self.pre_key = None
        self.last_d_pressed = False
        self.cmd_vel_x = 0.0
        self.cmd_vel_y = 0.0
        self.cmd_turn = 0.0
        # 传感器返回的机器人状态
    ...
    def _publish_thread(self):
        ...
        while self.pub_thread_running and not self.stop_event.is_set():
            ...
            # 累计目标时间，sleep 大部分留出 CPU 给 ROS，忙等收尾保证精度
            next_time += pub_interval
    ...
    def simulate_thread(self):
        ...
        # 控制参数
        time_step = 0
        sensor_frequency = 400  # 400Hz：对齐真机电机状态发布频率，保证仿真可以复现高频震荡

        target_time = 0
        _fall_log_interval = int(0.5 / self.model.opt.timestep)  # 跌倒后每0.5s打一次
        _fall_logged_step = -_fall_log_interval
        _pelvis_id = self.model.body("pelvis").id
        # last_control_time = 0
        try:
            while self.viewer.is_running() and not self.stop_event.is_set():
                ...
                with self.sim_view_lock:
                    ...
                    # 控制命令写入
                    self.set_motor_cmd()
                    # 处理来自 /sbus_data 的重置请求
                    if self.trigger_reset:
                        self._reset_robot()
                        self.trigger_reset = False
                    # 前进仿真一步：未启动或暂停时仅做 forward 保持可视化
                    if not self.paused and self.simulation_started:
                        mujoco.mj_step(self.model, self.data)
                        # ── 跌倒检测 ──────────────────────────────────────
                        pelvis_z = float(self.data.xpos[_pelvis_id, 2])
                        w, x, y, z = self.data.xquat[_pelvis_id]
                        roll, pitch, _ = quaternion_to_euler_scipy(w, x, y, z, degrees=True)
                        fallen = pelvis_z < 0.5 or abs(roll) > 60 or abs(pitch) > 60
                        if fallen and (time_step - _fall_logged_step) >= _fall_log_interval:
                            _fall_logged_step = time_step
                            print(f"[mujoco_node][FALL] t={target_time:.2f}s  "
                                  f"pelvis_z={pelvis_z:.3f}m  "
    ...
    def _elastic_cmd_callback(self, msg: String) -> None:
        """响应 sim_joy GUI 的弹性带控制命令。"""
        cmd = msg.data.strip().lower()
        if cmd == 'up':
            self.elastic_band.length = max(0.0, self.elastic_band.length - 0.1)
            print(f"[elastic_band] UP  length={self.elastic_band.length:.2f}")
        elif cmd == 'down':
            self.elastic_band.length += 0.1
            print(f"[elastic_band] DOWN length={self.elastic_band.length:.2f}")

    def _publish_elastic_status(self) -> None:
        """10Hz 发布弹性带状态供 GUI 显示。"""
        if not hasattr(self, '_elastic_status_pub'):
            return
        state = 'suspended' if self.elastic_band.enable else 'free'
        msg = String()
        msg.data = f'{state}:{self.elastic_band.length:.2f}'
        self._elastic_status_pub.publish(msg)

    def _joy_callback(self, msg: Joy):
        """响应 /sbus_data：A 启动仿真，C 进入停止，C 后 D 触发重置。"""
        axes = msg.axes
        n = len(axes)
        if n > 3:
            self.cmd_vel_x = axes[2]
            self.cmd_vel_y = axes[3]
            self.cmd_turn = axes[1]

        # A 键：axes[8] 按下即触发 simulation_started
        a_pressed = n > 8 and axes[8] > 0.5
        if a_pressed and self.elastic_band.enable and self.pre_key == "D":
            print("[mujoco_node] 仿真启动 (A) — 释放弹性带")
            self.simulation_started = True
            self.elastic_band.enable = False
            self.pre_key = 'A'

        # C 键：axes[10] 进入停止前置
        if n > 10 and axes[10] > 0.5:
            if self.pre_key != 'C':
                print("[mujoco_node] 收到 STOP 前置 (C)")
            self.pre_key = 'C'

        # D 键：axes[11]
        d_pressed = n > 11 and axes[11] > 0.5
        f_down = n > 7 and axes[7] > 0.5  # F 下拨 axes[7] == -1.0

        # F下拨 + D 上升沿：启动仿真（不释放弹力带）
        if f_down and d_pressed and not self.last_d_pressed and not self.simulation_started:
            print("[mujoco_node] 仿真启动 (F下+D) — 不释放弹力带")
            self.simulation_started = True
            self.pre_key = 'D'

        # 仅在 C 之后的 D 上升沿触发重置
        elif self.pre_key == 'C' and d_pressed and not self.last_d_pressed:
            print("[mujoco_node] 触发 reset (C->D) — 重新挂起弹性带")
            self.pre_key = 'D'
            self.simulation_started = False
            self.trigger_reset = True
            self.elastic_band.enable = True
        self.last_d_pressed = d_pressed

    def _reset_robot(self):
        """全量重置仿真状态，等同参考实现 mujoco_node.cpp reset_robot()。"""
        # 1. 全量清零（等同 mj_resetData：qpos 回 keyframe 0 或模型默认值，qvel/ctrl/act 归零）
        mujoco.mj_resetData(self.model, self.data)

        # 2. 所有 actuator 命令也清零，防止 rl_control 上次发的命令残留驱动关节
        self.data.ctrl[:] = 0.0

        # 3. 把浮基拨到悬挂高度（等同参考实现里第二次覆写 qpos）
        for j in range(self.model.njnt):
            if self.model.jnt_type[j] == mujoco.mjtJoint.mjJNT_FREE:
                qpos_adr = self.model.jnt_qposadr[j]
                self.data.qpos[qpos_adr + 0] = 0.0
                self.data.qpos[qpos_adr + 1] = 0.0
                self.data.qpos[qpos_adr + 2] = 1.10   # 悬挂高度
                self.data.qpos[qpos_adr + 3] = 1.0    # 四元数 w=1（无旋转）
                self.data.qpos[qpos_adr + 4] = 0.0
                self.data.qpos[qpos_adr + 5] = 0.0
                self.data.qpos[qpos_adr + 6] = 0.0
                break

        # 4. 更新运动学，让 viewer 立刻呈现新位姿
        mujoco.mj_forward(self.model, self.data)
    ...
    def start(self):
        ...
        try:
            ...
            # self.start_keyboard_listener()  # 禁用 mujoco 本地键盘控制，全部由 /sbus_data 接管
            viewer_thread.start()
            sim_thread.start()
            # 启动发布线程
...
def main():
    ...
    # 解析模型路径：优先从已安装的 share 目录读取，回退到源码目录
    try:
        share_dir = get_package_share_directory('mujoco_node')
        model_path = os.path.abspath(os.path.join(share_dir, model_registry[args.model]))
    except Exception:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.abspath(os.path.join(script_dir, '..', model_registry[args.model]))

    if not os.path.exists(model_path):
        # 兜底再试一次源码目录布局
        script_dir = os.path.dirname(os.path.abspath(__file__))
        alt = os.path.abspath(os.path.join(script_dir, '..', model_registry[args.model]))
    ...
    spin_thread.start()
    simulator = RobotSimulator(model_path, node, robot_config=args.config)
